    ...
    <div class="flex-col space-y-4 p-4">
        ...
        <div>
            ...
            <div class="rounded shadow-inner grid grid-cols-4 gap-3 relative bg-gray-100 p-4">
                ...
                <x-admin.form.input-group label="Date" for="filter-datefrom">
                    <input type="date" wire:model.lazy="filters.datefrom" name="datefrom" id="filter-datefrom"  />
        ...
        <x-admin.table.table>
            ...
            <x-admin.table.heading class="w-8">
                ...
                <x-admin.table.heading class="w-12">Réf. transaction</x-admin.table.heading>
                <x-admin.table.heading class="w-12">Montant</x-admin.table.heading>
                ...
                <x-admin.table.heading sortable wire:click="sortBy('quote_nom')" :direction="$sorts['quote_nom'] ?? null">
                    Nom
                </x-admin.table.heading>
            ...
            @if($selectPage)
                <x-admin.table.row>
                    <x-admin.table.cell colspan="6"
                        ...
                        @unless($selectAll)
                        <div>
                            <span>Vous avez sélectionné <strong>{{count($selected)}}</strong> commandes.
                                Voulez-vous
                                toutes les <strong>{{$commandes->total()}}</strong> commandes ?</span>
                            <x-button wire:click="selectAll"
                                      type="button">Sélectionner tout</x-button>
                        </div>
                        @else
                        <span>Vous avez sélectionné toutes les <strong>{{$commandes->total()}}</strong>
                            commandes</span>
                        @endif
                    </x-admin.table.cell>
                </x-admin.table.row>
                ...
                <x-admin.table.cell>
                    ...
                    <x-admin.table.cell><span class="text-xs"> <?php echo number_format(($commande->montant) / 100); ?></span></x-admin.table.cell>
